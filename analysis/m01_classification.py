...
appears on both sides of it. The outcome is rare, so the class balance is
corrected inside each training fold and never in the data used to score.
...
reference. The last stage reproduces the ordering defect the coursework carried,
balancing the classes before splitting, and measures how much it inflates the
result.
"""
...
def balance(block: pandas.DataFrame, seed: int) -> pandas.DataFrame:
    ...
    This is applied to a training fold and to the training partition, and never
    to anything a result is read from. The network's training loop stops on a
    validation accuracy, and on an outcome this rare that accuracy is maximized
    by predicting no readmission at all, so an unbalanced fold stops the loop
    before the network has learned anything.
    """
...
def search(train: pandas.DataFrame, features: list[str], categorical: list[str],
    ...
    the minority class is oversampled inside each training fold only. Balancing
    the training partition first and folding it afterwards would place copies of
    one encounter in the training fold and the validation fold together, which
    is the defect the last stage of this script measures.
    ...
    for parameters in grid:
        ...
        for fold_train, fold_validation in folds:
            model = pipeline(
                MLPClassifier(max_iter=config.MLP_MAX_ITER, early_stopping=True,
                              n_iter_no_change=10, random_state=config.SEED,
                              **parameters),
                categorical, numeric)
            fold = balance(train.iloc[fold_train], config.SEED)
        ...
        row = {"hidden_layers": len(parameters["hidden_layer_sizes"]),
        ...
        rows.append(row)
        print("  {:>10} alpha={:<8} cv ROC-AUC {:.4f}".format(
            row["layer_widths"], row["alpha"], row["cv_roc_auc_mean"]))
        if best is None or row["cv_roc_auc_mean"] > best[0]["cv_roc_auc_mean"]:
            best = (row, parameters)
            best_out_of_fold = out_of_fold
    return {"row": best[0], "parameters": best[1]}, rows, best_out_of_fold
...
def main() -> int:
    ...
    with utils.Timer(record, "m01"):
        ...
        # --- the grid search -------------------------------------------------
        print("grid search over {} configurations, {} folds each".format(
            len(config.MLP_GRID["hidden_layer_sizes"])
            * len(config.MLP_GRID["alpha"]), config.CV_FOLDS))
        ...
        # partition and applied unchanged to the test partition.
        threshold = evaluate.best_threshold(train[LABEL], out_of_fold)
        record.set(PREFIX + "threshold_source", "training out-of-fold scores")
        ...
        network = pipeline(
            MLPClassifier(max_iter=config.MLP_MAX_ITER, early_stopping=True,
                          ...
                          **best["parameters"]),
            categorical, numeric)
        network.fit(balanced[features], balanced[LABEL])
        record.set(PREFIX + "network_iterations",
                   int(network.named_steps["model"].n_iter_))
        test_score = network.predict_proba(test[features])[:, 1]
        record.update(evaluate.classification(test[LABEL], test_score, threshold),
                      PREFIX + "network.test_")

        # --- what the balancing costs ----------------------------------------
        # The same architecture on the unbalanced training partition, scored on
        # the same held-out encounters at the same threshold. The oversampling
        # is the step the coursework took and this rebuild kept, so what it does
        # to the result is measured and not asserted.
        unbalanced = pipeline(
            MLPClassifier(max_iter=config.MLP_MAX_ITER, early_stopping=True,
                          n_iter_no_change=10, random_state=config.SEED,
                          **best["parameters"]),
            categorical, numeric)
        unbalanced.fit(train[features], train[LABEL])
        record.set(PREFIX + "unbalanced.iterations",
                   int(unbalanced.named_steps["model"].n_iter_))
        record.update(
            evaluate.classification(
                test[LABEL],
                unbalanced.predict_proba(test[features])[:, 1], threshold),
            PREFIX + "unbalanced.test_")

        # --- the reference model ---------------------------------------------
        # Logistic regression on the same features, the same partition and the
        # same balancing. It is reported because a network that does not beat it
        # has not earned the extra capacity.
        reference = pipeline(LogisticRegression(max_iter=1000,
                                                random_state=config.SEED),
                             categorical, numeric)
        reference.fit(balanced[features], balanced[LABEL])
        reference_score = reference.predict_proba(test[features])[:, 1]
        record.update(
            evaluate.classification(test[LABEL], reference_score, threshold),
            PREFIX + "logistic.test_")

        utils.write_table(
            [{"model": name,
              **{key: round(record.number(PREFIX + prefix + key), 6)
                 for key in ("roc_auc", "average_precision", "accuracy",
                             "balanced_accuracy", "precision", "recall", "f1",
                             "brier")}}
             for name, prefix in (
                 ("network, balanced training", "network.test_"),
                 ("network, unbalanced training", "unbalanced.test_"),
                 ("logistic regression, balanced training", "logistic.test_"))],
        ...
        # importance and both appear unimportant.
        sample = test if len(test) <= IMPORTANCE_SAMPLE else train_test_split(
            test, train_size=IMPORTANCE_SAMPLE, random_state=config.SEED,
            stratify=test[LABEL])[0]
        record.set(PREFIX + "importance_rows", len(sample))
        importance = permutation_importance(
            network, sample[features], sample[LABEL],
        ...
        # encounters sit on both sides regardless. The identical model is
        # fitted under that ordering and scored on the partition that ordering
        # produces, so the difference is attributable to the ordering alone.
        ...
        utils.write_table(
            [{"partition": "patients held apart, balance inside training folds",
              "roc_auc": round(record.number(PREFIX + "network.test_roc_auc"), 6),
              "accuracy": round(
                  record.number(PREFIX + "network.test_accuracy"), 6),
    ...
    record.save()
    print("model 1: test ROC-AUC {:.4f}, average precision {:.4f}; the "
          "coursework ordering reports {:.4f}".format(
